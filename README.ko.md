...
- Run별 input／output／total token과 Run 기록 보존 기간에 관계없이 별도로 저장하는 Session 합계를 제공합니다. 기본·요약·대체 모델의 사용량을 모델별로 기록하여 비용을 추정하며, 가격이 없는 모델은 token만 표시합니다.
- Sandbox와 실행 승인을 적용한 파일, 문서, Shell, SSH 및 계획 도구.
- 메모리 격리 프로젝트는 대화, 계획, 첨부 및 작업 파일을 전용 RAM 디스크에 저장합니다. 단일 격리 작업 영역 안으로 백엔드가 제한하는 도구만 개별 승인이 면제됩니다. Shell, SSH, MCP, 외부 변환 및 렌더링은 면제되지 않습니다. 실험적 기능에서 새 프로젝트 생성을 꺼도 기존 프로젝트는 유지됩니다.
...
이 프로젝트는 [NR-Intern 라이선스 v1.1](LICENSE.ko.md)에 따라 제공됩니다.
사용 및 별도 라이선스 범위는 원문과 [상업용 라이선스 안내](COMMERCIAL-LICENSE.md)를 참조하십시오.

...
- Run ごとの input／output／total token と、Run 履歴の保持期間とは独立して保存する Session 累計。メイン・要約・フォールバックの使用量をモデル別に記録してコストを推定し、単価未設定のモデルは token のみ表示します。
- Sandbox と実行承認で保護されたファイル、文書、Shell、SSH、計画ツール。
- メモリ分離プロジェクトは会話、計画、添付、作業ファイルを専用 RAM ディスクに保存します。都度承認の免除は、単一の分離作業領域内にバックエンドが制限するツールだけです。Shell、SSH、MCP、外部変換・レンダリングは免除されません。実験的機能で新規作成を無効にしても既存のプロジェクトは維持されます。
...
本プロジェクトは [NR-Intern ライセンス v1.1](LICENSE.ja.md) に基づき提供されます。
利用と別途ライセンスの範囲は原文および [商用ライセンスの説明](COMMERCIAL-LICENSE.md) を参照してください。

...
- 只有 SDK 明確指出派送前連線已關閉，或本機 dial 失敗，才視為未派送並允許重連重試；
  不以 `session not found` 等錯誤文字推定遠端未執行。其餘連線錯誤只對 Server 宣告
  idempotent 的工具重試；未知結果須先查證，不能直接重送。
...
- 有副作用的 MCP 工具仍受人工 Approval 控制，不因 Project 使用 RAM Disk 而豁免。
  原生工具另受 elevated profile 控制；MCP 不宣稱具有相同的本機檔案沙箱。
- 相同操作的先前結果未知時，只能逐次人工核准重試，Session 永久核准不能略過。
...
模型可用 `find_tools` 以關鍵字（中英文皆可）把工具連同參數 schema 取回。模型在本 Run
尚未看過目前契約時，直接呼叫會先拒絕派送並於下一輪公開定義；看過且契約未變的工具可續用。
呼叫過的工具會留在該 Run 的目錄裡。查詢會帶上同一個 session
...
工具回傳 `input_required` 時，模型取得輸入請求及 `_mcp_continuation_id`，保留原始業務參數，
以 `_mcp_input_responses` 回覆。opaque `requestState` 只留在 Runtime 記憶體，不送入模型或紀錄。
續接 ID 綁定 Session、Server、工具、原參數與契約版本，單次使用、24 小時到期；最多保存
512 筆，每筆狀態上限 64 KiB。仍待追加輸入的相同操作不重開，程式重啟後舊 ID 失效。

每輪目錄、核准前及派送前皆核對契約；重新連線後若 schema、唯讀／授權屬性或來源版本改變，
停止派送並重新確認。這不能保證遠端實作符合其自述，也不能把未知執行結果當成未執行。
...
工具目錄每一輪都會整份進入提示，但幾百條扁平的名稱與描述很難讓模型判斷「這件事該用
哪一組工具」。實測過的後果是它挑錯工具、或反覆呼叫不適合的那個，每次都要燒掉一整輪。

因此**安裝完成時會自動把契約讀過一遍**，MCP 管理頁的「詳讀契約」按鈕也可以隨時重讀。
流程是：重新連線取得完整工具定義（描述、必填參數、唯讀屬性）→ 依字元預算分批交給
摘要 Provider 整理 → 合併成一份使用導覽 → 寫入**跨專案共用**的記憶 scope。

幾個刻意的取捨：

- **分批而不是一次送完**：幾百個工具的描述會超過小型本機模型的視窗，而這個功能對本機
  模型同樣要能用。分批時讓同一個命名前綴盡量留在同一批——前綴通常對應 Server 的模組
  分組，拆開會讓模型在兩批各看到半組，得出沒有用的描述。
- **不送完整 InputSchema**：判斷「什麼時候用這個工具」需要的是必填參數與描述，不是型別
  細節；必填參數尤其重要，那是「需要先有某個 ID」這類前置條件唯一看得出來的地方。
- **寫成 `procedure` 而不是 `decision`**：這是「怎麼用這台 Server」的操作知識，不是某次
  判斷的結果。
- **scope 不綁專案**：MCP Server 是外部資源，甲專案讀出來的用法對乙專案同樣成立，鎖在
  專案裡等於每個專案各讀一次。
- **摘要有長度上限**：記憶每一輪都會進提示，寫太長等於把原本的問題換個地方重演。
- **Server 自述的 instructions 以外部資料邊界標示後才送入**，不視為指令——那是一條現成的
  提示注入路徑。

安裝時的自動解讀是靜默的：沒有可用 Provider 或回憶空間停用時只會略過，不會讓安裝看起來
失敗——MCP Server 本身已經存好了。

...
LOOP 要解決的是「Agent 跑一半就忘了」，不是自動化便利。反覆叫 Agent「繼續」的典型失敗不是
...
- **次數不是步驟。** 這裡的命名是踩過坑換來的。功能原本叫「多輪執行」、計數叫「第 N 輪」，
  而「輪」正是使用者領域裡表示「反覆掃過同一批問題」的字（例如輸入法演算法的「五輪修正」）。
  兩者撞字，Agent 於是把計畫的 5 個步驟命名成「第 1～5 輪」。
  中間一度改用**禁止那種命名**來擋，結果誤傷了使用者自己的用語——那個「五輪」本來就出自
  使用者的提示，Agent 並沒有理解錯。禁止規則已經移除。
...
- **空轉以步驟指紋判定**（`PlanStepFingerprint`，只看狀態與證據）：改寫標題不算推進，
  「我來確認一下」寫得再長也不是。連續 2 次沒推進即停止，並說明是空轉而非完成。

`plan_loop_interrupt` 讓 Agent **立刻**中止當前這一次（真的取消該 Run），同時寫入

...
多輪要解決的是「Agent 跑一半就忘了」，不是自動化便利。反覆叫 Agent「繼續」的典型失敗不是
它停下來，而是它繼續得很起勁卻在做別的事：忘記原始目標、每輪重新鋪陳一次計畫、沒有證據
就宣告完成。這些看起來都像在工作。因此設計原則是——**任務的真相在計畫裡，不在模型的
記憶裡**，而每一項限制都指向這句話：

- **一輪 = 一次 Run。** 不做成「一個 Run 內跑很多輪」：核准、取消、Context 整理、
...
- **每輪輸入由後端從儲存重新組裝**（`domain.PlanRoundBrief`）：目標逐字、每個步驟的狀態與
  證據、第 N 輪／共 M 輪。不沿用模型上一輪的說法——複述是走鐘的傳染途徑，第 3 輪照著第 2
  輪的複述工作，就跟原始目標無關了。唯一的例外是中斷時留下的交接內容。
- **輪不是步驟。** 「第 N 輪」緊接著編號步驟清單，很容易被讀成「第 N 輪＝做第 N 步」；
  實際觀察到的誤解正是這個——Agent 把計畫的 5 個步驟命名成「第 1～5 輪」。因此每輪輸入
  在步驟清單**之前**就先講明兩者無關，`plan_create` 的工具說明也一併澄清：Agent 設計計畫
  時只讀得到那段說明，多輪的每輪輸入要等多輪跑起來才看得到。
  說明擋不住時就用強制：`domain.NewPlan` 對 **Agent 建立**的計畫拒絕「第 N 輪」式的步驟
  標題（`StepTitleLooksLikeRound`），錯誤訊息說明正確的機制是什麼。只比對標題開頭，
  「輪詢遠端狀態」這類真實工作不受影響；使用者自己建立的計畫不受此限。
- **完成不能自我宣告。** 步驟要 `completed` 必須有證據（`TransitionPlanStep` 早有此限制），
  全部結束才算完成。Agent 沒有任何工具可以宣告做完了。
- **多輪期間禁止 `plan_create`。** 這是走鐘最直接的路徑：開一個新計畫，多輪會很開心地繼續
  跑，跑的卻是另一個任務、輪數還在算。步驟仍可用 `plan_step_update` 調整。
- **輪數是預算。** `Round` 記的是「已開始」而非已完成——中斷的那一輪照樣計入，
  否則反覆中斷再續跑就能無限延長。上限只有使用者能設，硬上限 20。
- **空轉以步驟指紋判定**（`PlanStepFingerprint`，只看狀態與證據）：改寫標題不算推進，
  「我來確認一下」寫得再長也不是。連續 2 輪沒推進即停止，並說明是空轉而非完成。

`plan_loop_interrupt` 讓 Agent **立刻**中止當前輪（真的取消該 Run），同時寫入
`PlanCheckpoint`：輪次、進行中的步驟、以及它自述做到哪裡。中止會留下寫到一半的計畫——
...
有額度時，Provider 設定會出現一張可收合的面板，**逐筆**列出可用額度與各自的到期時間，
每筆一顆兌換按鈕。只給總數與一顆按鈕等於替使用者決定用掉哪一筆——各筆到期時間不同，
那個選擇應該留給他。按下仍需二次確認，確認框寫明是哪一筆。時間以當地時區顯示、不附時區
位移：使用者看的是自己機器的時間，GMT 位移只是雜訊。兌換會消耗帳號有限

...
- 記憶體隔離專案：對話、計畫、附件與工作檔案**執行期間就寫在 RAM Disk 上**，不經過硬碟，關閉或重啟即消失，只保留 Project 設定；每個 Project 使用獨立、可調容量的 RAM Disk Sandbox。這類 Project 的工具呼叫不逐次詢問核准；可在實驗性功能中關閉新建（預設開啟），既有 Project 不受影響。
